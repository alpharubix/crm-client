...
const PERIOD_PRESETS: { id: PeriodPresetId; label: string }[] = [
...
export default function SupportTicketsPage() {
  ...
  const fetchTickets = async () => {
    ...
    try {
      const res = await fetch(
        `${ENV.VITE_BACKEND_BASE_URL}/v1/support-ticket/history`,
        {
          credentials: 'include',
        },
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(
        `${ENV.VITE_BACKEND_BASE_URL}/v1/support-ticket/create`,
        {
          method: 'POST',
          headers: {
      ...
      if (!res.ok) {
        throw new Error(
          data.detail || data.message || 'Failed to submit ticket',
        );
      }

      toast.success(
        `Support Ticket ${data?.data?.ticket_id || ''} created successfully!`,
      );
      setTitle('');
      setDescription('');
  ...
  const handleStatusChange = async (ticketId: string, newStatus: string) => {
    ...
    try {
      const res = await fetch(
        `${ENV.VITE_BACKEND_BASE_URL}/v1/support-ticket/${ticketId}/status`,
        {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (!res.ok) {
        throw new Error(
          data.detail || data.message || 'Failed to update ticket status',
        );
      }
  ...
  const filteredTickets = useMemo(() => {
    return tickets.filter((t) => {
      ...
      const matchesStatus =
        ...
        (targetStatus === 'IN_PROGRESS' && currentStatus === 'INPROGRESS') ||
        (targetStatus === 'INPROGRESS' && currentStatus === 'IN_PROGRESS');
      ...
      return (
        matchesSearch && matchesStatus && matchesPriority && matchesPeriod
      );
    });
  }, [
    tickets,
      ...
      case 'high':
        return (
          <Badge className='bg-orange-100 text-orange-700 border-orange-200 hover:bg-orange-200 font-semibold px-2.5 py-0.5 shadow-none'>
            <span className='w-1.5 h-1.5 rounded-full bg-orange-600 mr-1.5' />{' '}
            High
          </Badge>
        );
      case 'medium':
      ...
      <div className='flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white shadow-xl relative overflow-hidden'>
        ...
        <div className='space-y-1.5 relative z-10'>
          <div className='flex items-center gap-2'>
            ...
            {isAdmin && (
              <span className='px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-500/20 text-purple-300 border border-purple-400/30 backdrop-blur-md flex items-center gap-1'>
                <ShieldCheck className='w-3 h-3 text-purple-300' /> Super Admin
                Access
              </span>
            )}
          </div>
          ...
          <p className='text-slate-300 text-sm max-w-xl'>
            Raise software issues, submit bug reports for technical modules, and
            track status resolution in real-time.
          </p>
        </div>
        ...
        <div className='flex items-center gap-3 relative z-10'>
          ...
          >
            <RefreshCw
              className={`w-4 h-4 mr-2 ${isLoadingTickets ? 'animate-spin' : ''}`}
            />
            Refresh Desk
          </Button>
        </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-8 items-start'>
            ...
            <form onSubmit={handleSubmit} className='space-y-4'>
              ...
              <div className='space-y-1.5'>
                <Label
                  ...
                  className='text-xs font-semibold text-slate-700 flex items-center gap-1'
                >
                  <Layers className='w-3.5 h-3.5 text-slate-400' /> Affected
                  Module
                </Label>
                <Select
                  value={service}
        ...
        <div className='lg:col-span-8 space-y-4'>
          ...
          <div className='p-4 bg-white rounded-2xl border border-slate-200/80 shadow-sm space-y-3.5'>
            {/* Top Row: Search Input + Status Filter + Priority Filter + Reset */}
            <div className='flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3'>
              ...
              <div className='relative flex-1 min-w-[200px]'>
                ...
                <Input
                  ...
                  className='pl-9 pr-8 text-xs rounded-xl border-slate-200 bg-slate-50/60 focus:bg-white transition-all h-9'
                />
                {searchQuery && (
                  <button
                    type='button'
              ...
              {/* Status and Priority Filter Selectors */}
              <div className='flex items-center gap-2 flex-wrap sm:flex-nowrap justify-end'>
                <div className='flex items-center gap-1.5'>
                  <Filter className='w-3.5 h-3.5 text-slate-400 hidden sm:block' />
                  <Select value={statusFilter} onValueChange={setStatusFilter}>
                    <SelectTrigger className='h-9 text-xs rounded-xl w-32 sm:w-34 border-slate-200 bg-slate-50/60 focus:bg-white font-medium'>
                ...
                {hasActiveFilters && (
                  <Button
                    ...
                    className='h-9 px-2.5 text-xs text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-xl transition-colors cursor-pointer shrink-0'
            ...
            {/* Bottom Row: Period-wise Date Filter (From Date to To Date) + Quick Presets */}
            <div className='pt-3 border-t border-slate-100 flex flex-col xl:flex-row xl:items-center justify-between gap-3'>
              ...
              <div className='flex items-center gap-2.5 flex-wrap'>
                <div className='flex items-center gap-1.5 text-slate-700 mr-1'>
                ...
                <div className='flex items-center gap-1.5'>
                  ...
                  <div className='flex items-center gap-1 bg-slate-50/70 border border-slate-200/80 rounded-lg px-2 h-8 hover:bg-slate-100/50 transition-colors'>
                    ...
                    <input
                      ...
                      className='bg-transparent text-xs text-slate-700 outline-none w-18 cursor-pointer font-medium'
                ...
                {(fromDate || toDate || fromTime || toTime) && (
                  <button
                    ...
                    className='p-1 rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100 text-xs transition-colors'
          ...
          <Card className='border border-slate-200/80 shadow-sm rounded-2xl bg-white overflow-hidden'>
            <CardHeader className='py-4 px-6 border-b border-slate-100 flex flex-row items-center justify-between'>
              <div>
                ...
                <CardDescription className='text-xs text-slate-500'>
                  Showing {filteredTickets.length} of {tickets.length} total
                  tickets
                  {hasActiveFilters && (
              ...
              {hasActiveFilters && (
                <Button
                  ...
                  size='sm'
                  onClick={handleClearFilters}
                  className='h-7 text-xs text-slate-600 hover:text-slate-900 border-slate-200 rounded-lg'
                >
                  Clear Filters
                </Button>
              )}
              ...
              {isLoadingTickets && tickets.length === 0 ? (
                ...
                <div className='text-center py-16 text-slate-500'>
                  ...
                  <p className='text-xs text-slate-400 mt-1 max-w-sm mx-auto'>
                    {hasActiveFilters
                      ? 'No tickets match the selected period, filters, or search query.'
                  ...
                  {hasActiveFilters && (
                    <Button
                      ...
                      size='sm'
                      onClick={handleClearFilters}
                      className='mt-3 h-8 text-xs text-blue-600 border-blue-200 hover:bg-blue-50 rounded-lg'
                    >
                      Reset all filters
                    </Button>
                  )}
                            ...
                            <TableCell className='py-3.5 max-w-xs'>
                              ...
                              <span className='text-xs text-slate-400 block mt-0.5 flex items-center gap-1.5 flex-wrap'>
                                ...
                                {t.user_name && (
                                  <>
                                    <span>•</span>
                                    <span>Raised by: <span className='font-medium text-slate-600'>{t.user_name}</span></span>
                                  </>
                                )}
                              </span>
                            </TableCell>
