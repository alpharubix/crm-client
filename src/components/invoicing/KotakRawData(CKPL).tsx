...
function mapBackendToTransaction(d: any): TransactionData {
  return {
    ...
    invoiceDate: d.invoice_date_id || d.invoice_date || "",
    disbursementDate: d.disbursement_date || "",
    tranValueAmount: parseNumber(d.tran_value_amount || d.transaction_amount),
    balanceOutstanding: parseNumber(d.balance_outstanding_inr || d.balance_outstanding),
    dueDate: d.due_date || "",
    customerPaymentDate: d.customer_payment_date || "",
    overdueWithinCureInr: parseNumber(d.overdue_within_cure_inr || d.overdue_within_cure_amount),
    overdueWithinCureDays: parseNumber(d.overdue_within_cure_no_of_days || d.overdue_within_cure_days),
    overdueBeyondCureInr: parseNumber(d.overdue_beyondcure_inr || d.overdue_beyond_cure_amount),
    overdueBeyondCureDays: parseNumber(d.overdue_beyond_cure_no_of_days || d.overdue_beyond_cure_days),
    location: d.location || "",
...
export function KotakRawDataCKPL() {
  ...
  async function fetchData(subTab: "transaction" | "limit", pageNumber: number, silent = false) {
    try {
      ...
      const url = `${ENV.VITE_BACKEND_BASE_URL}/invoice/kotak-ckpl/${endpoint}?${params.toString()}`;
      ...
      if (subTab === "transaction") {
        const rawList = json.data?.kotak_ckpl_transactions || [];
        setTransactions(rawList.map(mapBackendToTransaction));
      } else {
        const rawList = json.data?.kotak_ckpl_credits || [];
        setLimits(rawList.map(mapBackendToCreditLimit));
      }
    } catch (err: any) {
      console.error("Fetch error:", err);
      toast.error(err.message || "Failed to fetch Kotak CKPL records");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function performUpload(file: File) {
    ...
    try {
      ...
      const url = `${ENV.VITE_BACKEND_BASE_URL}/invoice/kotak-ckpl/${endpoint}`;
  ...
  async function handleExport() {
    try {
      ...
      const fileLabel = activeSubTab === "transaction" ? "kotak-ckpl-transactions" : "kotak-ckpl-limits";
              ...
              {loading ? (
                ...
                filteredTransactions.map((tx) => (
                  <TableRow key={tx.id}>
                    ...
                    <TableCell className="text-right">{formatCurrency(tx.overdueWithinCureInr)}</TableCell>
                    <TableCell className="text-right">{tx.overdueWithinCureDays}</TableCell>
                    <TableCell className="text-right">{formatCurrency(tx.overdueBeyondCureInr)}</TableCell>
                    <TableCell className="text-right">{tx.overdueBeyondCureDays}</TableCell>
      ...
      {uploadLoading && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-black/40 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="bg-white rounded-xl shadow-lg border border-slate-200 p-6 flex flex-col items-center gap-4">
            ...
            <span className="text-sm font-semibold text-slate-700">Uploading and processing Kotak CKPL file...</span>
          </div>
        </div>
      )}
